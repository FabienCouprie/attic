...
// Paulstretch : étirement extrême avec randomisation de phase.
export function paulstretch(buffer: AudioBuffer, facteur: number): AudioBuffer {
  const sr = buffer.sampleRate;
  const tailleFenetre = Math.max(256, Math.round(sr * 0.25));
  const saut = Math.max(1, Math.round(tailleFenetre / facteur));
  const fenetre = new Float64Array(tailleFenetre);
  for (let i = 0; i < tailleFenetre; i++) {
    fenetre[i] = 0.5 - 0.5 * Math.cos((2 * Math.PI * i) / (tailleFenetre - 1));
  }
  const longueurSortie = Math.max(tailleFenetre, Math.round(buffer.length * facteur));
  const resultat = new AudioBuffer({ numberOfChannels: buffer.numberOfChannels, length: longueurSortie, sampleRate: sr });
  const nbBins = tailleFenetre / 2 + 1;
  ...
  for (let c = 0; c < buffer.numberOfChannels; c++) {
    ...
    while (pos < buffer.length) {
      const re = new Float64Array(tailleFenetre);
      const im = new Float64Array(tailleFenetre);
      for (let i = 0; i < tailleFenetre; i++) {
        const idx = pos + i;
        re[i] = (idx < src.length ? src[idx] : 0) * fenetre[i];
      }
      const oldRe = new Float64Array(tailleFenetre);
      const oldIm = new Float64Array(tailleFenetre);
      if (pos > 0) {
        for (let i = 0; i < tailleFenetre; i++) {
          const idx = pos - saut + i;
          oldRe[i] = (idx >= 0 && idx < src.length ? src[idx] : 0) * fenetre[i];
        }
        fft(oldRe, oldIm, false);
      }
      fft(re, im, false);
      for (let b = 0; b < nbBins; b++) {
        const mag = Math.hypot(re[b], im[b]);
        const oldMag = Math.hypot(oldRe[b], oldIm[b]);
        const oldPhase = Math.atan2(oldIm[b], oldRe[b]);
        const randPhase = Math.random() * 2 * Math.PI;
        const mixPhase = pos > 0 ? oldPhase * 0.5 + randPhase * 0.5 : randPhase;
        const finalMag = pos > 0 ? mag * 0.5 + oldMag * 0.5 : mag;
        re[b] = finalMag * Math.cos(mixPhase);
        im[b] = finalMag * Math.sin(mixPhase);
        if (b > 0 && b < tailleFenetre - b) {
          re[tailleFenetre - b] = re[b];
          im[tailleFenetre - b] = -im[b];
        }
      }
      fft(re, im, true);
      for (let i = 0; i < tailleFenetre; i++) {
        const outPos = pos + i;
        if (outPos >= longueurSortie) break;
        sortie[outPos] += re[i] * fenetre[i];
        enveloppe[outPos] += fenetre[i] * fenetre[i];
      }
      pos += saut;
    }
