...
export async function bufferVersOggBlob(buffer: AudioBuffer): Promise<Blob> {
  const mimeTypes = [
  ...
  return new Promise((resolve, reject) => {
    const enreg = new MediaRecorder(dest.stream, { mimeType: mime });
    const troncons: Blob[] = [];
    enreg.ondataavailable = (e) => { if (e.data.size > 0) troncons.push(e.data); };
    enreg.onstop = () => {
      ctx.close();
      resolve(new Blob(troncons, { type: mime.includes("ogg") ? "audio/ogg" : "audio/webm" }));
    };
    enreg.onerror = (e) => { ctx.close(); reject(e); };
    enreg.start();
    source.start();
    source.onended = () => enreg.stop();
  });
}


export async function decoderAudioUrl(dataUrl: string, ctx: AudioContext): Promise<AudioBuffer> {
