...
//
// Grille : 8 pistes fixes (ordre) × N pas, bouclée sur `mesures` mesures.
//
// IL Y AVAIT DEUX SÉQUENCEURS DE BATTERIE, il n'en reste qu'un. Le premier jouait cinq pistes
// sans nuance, le second les mêmes cinq — dans le même ordre — plus trois, avec une vélocité par
// pas. Le premier était donc un sous-ensemble strict du second, à un détail près : il proposait une
// résolution de huit pas, que le second a reprise. Ce qu'il reste de lui ici est la LECTURE de ses
// motifs, pour que les graphes enregistrés qui le portent jouent encore — voir
// `decoderMotifVelocite`.
...
/**
 * Vélocité donnée à un pas venu d'un motif binaire.
 *
 * NEUF, et non six : la vélocité divise le niveau par neuf dans le rendu avancé, si bien que 9
 * reproduit EXACTEMENT le niveau de l'ancien nœud, qui n'avait pas de nuance. Un graphe enregistré
 * doit sonner comme avant, pas « comme avant en plus faible ».
 */
export const VELOCITE_MOTIF_BINAIRE = 9;

/**
 * Un motif de l'ancien séquenceur binaire est-il en train d'être relu ?
 *
 * LE SIGNAL EST LE NOMBRE DE LIGNES. La grille avancée en écrit toujours huit ; cinq ne peuvent
 * venir que de l'ancien nœud, supprimé. On ne se fie pas aux chiffres — un motif avancé n'ayant que
 * des 0 et des 1 est légitime, c'est un motif joué tout en douceur.
 */
export const estMotifBinaire = (motif: string, nRows: number): boolean => {
  const lignes = (motif || "").split("|");
  return lignes.length === PISTES_MOTIF_BINAIRE && nRows > PISTES_MOTIF_BINAIRE
    && lignes.every((l) => /^[01]*$/.test(l));
};

export function decoderMotifVelocite(motif: string, nRows: number, nbPas: number): number[][] {
  const lignes = (motif || "").split("|");
  // Un motif de l'ancien séquenceur : ses « 1 » deviennent des frappes à pleine vélocité, et ses
  // cinq pistes tombent sur les cinq premières de celui-ci — c'est le même ordre d'instruments.
  const binaire = estMotifBinaire(motif, nRows);
  const g: number[][] = [];
  for (let r = 0; r < nRows; r++) {
    ...
    for (let c = 0; c < nbPas; c++) {
      ...
      if (binaire) row.push(v === 1 ? VELOCITE_MOTIF_BINAIRE : 0);
      else row.push(v >= 1 && v <= 9 ? v : 0);
    }
    g.push(row);
  }
  return g;
