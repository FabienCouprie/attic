...
function bufferSinus(freq: number, dureeS: number, channels = 1): any {
...
describe("math-formules", () => {
  it("génère un sinus de 440 Hz", () => {
    ...
    expect(rms(b)).toBeGreaterThan(0.6);
    expect(rms(b)).toBeLessThanOrEqual(1);
  });
  ...
  it("applique une formule identité sans modifier le signal", () => {
    const src = bufferSinus(440, 0.05);
  ...
  it("ajoute une sinusoïde à un signal existant", () => {
    const src = bufferSinus(440, 0.05);
    const out = appliquerFormuleEchantillons(src, "x + sin(t * 2 * pi * 1000)");
