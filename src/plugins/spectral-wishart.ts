...
export const fiches: FicheAudio[] = ([
  ...
  },
  {
    ...
    notice: "D'après Trevor Wishart, « Audible Design » (1994), et le programme `blur freeze` du Composers Desktop Project.\n\nEn quoi cela diffère du gel granulaire, déjà présent. Celui-là boucle un morceau de signal : on entend la boucle, sa période et ses raccords, et le son garde le grain du bout qu'on a pris. Ici on tient une analyse — les énergies sont celles de l'instant choisi, mais les phases continuent d'avancer comme si le son se prolongeait. Il n'y a donc ni période, ni raccord, ni battement : le son ne bouge plus du tout.\n\nC'est la différence entre arrêter un disque sur un sillon et arrêter le temps. Ce qui précède l'instant choisi passe intact ; à partir de là, le son se fige et tient jusqu'à la fin.",
    noticeEn: "After Trevor Wishart, « Audible Design » (1994), and the Composers Desktop Project's `blur freeze`.\n\nHow it differs from the granular freeze already present. That one loops a piece of signal: one hears the loop, its period and its joins, and the sound keeps the grain of the fragment taken. Here an analysis is held — the energies are those of the chosen moment, but the phases keep advancing as though the sound went on. So there is no period, no join, no beating: the sound stops moving entirely.\n\nIt is the difference between stopping a record on a groove and stopping time. What precedes the chosen moment passes through untouched; from there on the sound freezes and holds to the end.",
    entrees: [{ nom: "Audio", type: "audio" }],
    sorties: [{ nom: "Audio", type: "audio" }],
    parametres: [
