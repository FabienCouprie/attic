...
export const fiches: FicheAudio[] = ([
  {
    ...
    notice: "D'après la synthèse par pulsars de Curtis Roads (« Microsound », 2001). Un pulsar est une brève forme d'onde — le pulsaret — suivie de silence, le tout répété. Deux durées le décrivent, et c'est là tout son intérêt : la période de répétition, dont l'inverse donne la fondamentale entendue, et la durée DU pulsaret, dont l'inverse donne la position du formant, c'est-à-dire la région du spectre où l'énergie se concentre. Ces deux grandeurs sont indépendantes : la note descend d'une octave sans que le formant bouge, et le formant se déplace sans que la note change. Aucun instrument acoustique ne le permet, et une granulation classique non plus, sa grille temporelle et le contenu de ses grains restant liés. Le silence entre pulsars rend ce découplage possible. Quand la durée du pulsaret atteint la période — c'est-à-dire quand le formant descend jusqu'à la fondamentale — les pulsars se touchent et le procédé se dissout en une forme d'onde continue ; le composant plafonne là, ce qui est une contrainte du modèle et non une limite de l'implémentation.",
    noticeEn: "After Curtis Roads's pulsar synthesis (\"Microsound\", 2001). A pulsar is a brief waveform — the pulsaret — followed by silence, the whole thing repeated. Two durations describe it, and therein lies its interest: the repetition period, whose inverse gives the fundamental you hear, and the pulsaret duration, whose inverse gives the formant position, that is, the region of the spectrum where energy concentrates. The two are independent: the note drops an octave without the formant moving, and the formant moves without the note changing. No acoustic instrument allows this, and neither does classic granulation, whose time grid and grain content stay tied together. The silence between pulsars is not incidental: it is what makes the decoupling possible. When the pulsaret duration reaches the period — that is, when the formant drops to the fundamental — pulsars touch and the process dissolves into a continuous waveform; the node caps there, which is a constraint of the model rather than a limitation of the implementation.",
    entrees: [],
