...
export const fiches: FicheAudio[] = ([
  {
    ...
    notice: "Réunit le grave des deux canaux sous une fréquence de coupure. Trois raisons à cela :\n• la gravure : un sillon de disque porte la somme des canaux sur un axe et leur différence sur l'autre, si bien qu'un grave décorrélé fait sauter le burin hors du sillon, et l'atelier refuse le disque ou en baisse le niveau\n• la salle : sous une centaine de hertz, la longueur d'onde dépasse trois mètres, l'oreille n'y localise plus rien, et deux graves différents ne produisent qu'un flottement d'énergie selon l'endroit où l'on se tient ; rien ne se perd à les réunir\n• la somme mono : c'est dans le grave que les annulations coûtent le plus cher, parce que c'est là qu'est l'énergie.\n\nLe traitement est un passe-haut sur le côté, et rien d'autre. Cette formulation rend l'opération exacte, là où un découpage de chaque canal en deux bandes ne l'est pas : la bande aiguë obtenue par soustraction garde énormément de grave (à 50 Hz sous une coupure à 120, la moitié de l'amplitude) parce qu'un filtre déphase et qu'une soustraction ne fait pas disparaître ce qui est déphasé.\n\nLe milieu n'est jamais filtré, si bien que la somme mono ressort au bit près quel que soit le réglage. À quantité nulle, l'entrée ressort telle quelle, sans même un déphasage.\n\nLe composant dit ce qu'il a fait : de combien le côté grave a baissé, et ce que devient la corrélation des deux canaux sous la coupure. Le premier chiffre dépend de la distance à la coupure, comme tout filtre. Sous 120 Hz : −22,7 dB pour un côté à 50 et 70 Hz, −40,9 dB à 30 et 40 Hz.",
    noticeEn: "Collapses the bass of both channels below a crossover frequency. Three reasons for it:\n• cutting: a record groove carries the sum of the channels on one axis and their difference on the other, so a decorrelated bass throws the cutting stylus out of the groove, and the plant refuses the record or lowers its level\n• the room: below a hundred hertz or so, the wavelength exceeds three metres, the ear localises nothing there, and two different basses only produce an energy flutter depending on where one stands; nothing is lost by joining them\n• the mono sum: it is in the bass that cancellations cost the most, because that is where the energy is.\n\nThe processing is a high-pass on the side, and nothing else. That formulation makes the operation exact, where splitting each channel into two bands does not: the high band obtained by subtraction keeps a great deal of bass (at 50 Hz under a 120 Hz crossover, half the amplitude) because a filter shifts phase and a subtraction does not remove what has been phase-shifted.\n\nThe mid is never filtered, so the mono sum comes out bit for bit whatever the setting. At zero amount the input comes out as it went in, without even a phase shift.\n\nThe node says what it did: by how much the low side fell, and what the correlation of the two channels below the crossover becomes. The first figure depends on the distance to the crossover, as with any filter. Under 120 Hz: -22.7 dB for a side at 50 and 70 Hz, -40.9 dB at 30 and 40 Hz.",
  ...
  },
  {
    ...
    notice: "Déplace le timbre d'un son vers celui d'un autre, par interpolation de leurs spectres. À chaque instant un seul son s'entend, dont le timbre se déplace : les deux sources ne se superposent jamais.\n\nElles ont le même rôle, et le réglage traverse continûment de l'une à l'autre. L'opération est donc symétrique, à la différence d'une synthèse croisée où l'enveloppe de l'un s'applique à l'autre et où la porteuse fournit seule la matière.\n\nL'interpolation se fait sur les logarithmes des amplitudes. Interpoler linéairement ferait dominer le plus fort des deux : à mi-chemin entre un partiel à 1 et un partiel à 0,01, la moyenne vaut 0,505, soit le son fort à un demi-décibel près. En logarithme, le même milieu donne 0,1, c'est-à-dire à mi-chemin en décibels, la seule façon d'être au milieu pour l'oreille.\n\nLa phase vient du son dominant, celui vers lequel on penche, et non d'une interpolation : deux phases moyennées ne donnent pas une phase intermédiaire mais une interférence, et le résultat s'amincit au lieu de se déplacer. Ce sont les amplitudes qui portent le timbre ; la phase porte le grain.\n\nLa méthode interpole les amplitudes case par case et n'apparie pas les partiels pour les faire glisser de l'un à l'autre. Entre un sinus à 200 Hz et un sinus à 3000, le milieu ne contient donc pas un sinus à 775 Hz : il contient les deux, à la moyenne géométrique de leurs amplitudes. Sur des sons riches, cela s'entend comme un timbre intermédiaire et le centroïde le confirme : il passe de 200 à 2640 puis à 3000 Hz à mesure qu'on traverse. Sur deux sinus isolés, la limite se voit.\n\nUne courbe branchée sur l'entrée Modulation fait traverser au fil du son : une rampe part du premier et arrive au second, un sinus fait l'aller-retour.",
    noticeEn: "Moves the timbre of one sound towards another, by interpolating their spectra. At any instant a single sound is heard, whose timbre moves: the two sources never overlap.\n\nThey have the same role, and the setting travels continuously from one to the other. The operation is therefore symmetric, unlike a cross-synthesis where the envelope of one is applied to the other and where the carrier alone provides the matter.\n\nThe interpolation is done on the logarithms of the amplitudes. Interpolating linearly would let the louder of the two dominate: halfway between a partial at 1 and a partial at 0.01, the mean is 0.505, that is the loud sound to within half a decibel. In logarithm the same midpoint gives 0.1, that is halfway in decibels, the only way to be in the middle for the ear.\n\nThe phase comes from the dominant sound, the one being leaned towards, rather than from an interpolation: two averaged phases do not make an intermediate phase but an interference, and the result thins out instead of moving. Amplitudes carry the timbre; phase carries the grain.\n\nThe method interpolates amplitudes bin by bin and does not pair partials to glide them from one to the other. Between a 200 Hz sine and a 3000 Hz sine the midpoint therefore does not hold a 775 Hz sine: it holds both, at the geometric mean of their amplitudes. On rich sounds this is heard as an intermediate timbre and the centroid confirms it: it goes from 200 to 2640 then to 3000 Hz as one travels. On two isolated sines, the limit shows.\n\nA curve connected to the Modulation input travels along the sound: a ramp starts at the first and arrives at the second, a sine goes there and back.",
    entrees: [
