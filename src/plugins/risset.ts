...
export const fiches: FicheAudio[] = ([
  {
    ...
    notice: "Illusion auditive décrite par Jean-Claude Risset, version continue du son étagé de Shepard — l'équivalent sonore de l'escalier sans fin d'Escher. Le composant superpose plusieurs copies du son fourni, espacées d'une octave, qui glissent toutes ensemble ; une copie qui atteint le haut de l'étendue s'est déjà éteinte, et celle qui réapparaît en bas est encore inaudible, si bien que le saut ne s'entend jamais. Chaque copie est relue à vitesse variable, comme une bande magnétique : la hauteur et le débit changent ensemble, et le son est bouclé pour alimenter le glissando aussi longtemps que demandé. Fonctionne d'autant mieux que la source est riche et peu rythmée — une nappe, une voix tenue, un bruit coloré ; sur une boucle très marquée, on entend surtout la boucle.",
