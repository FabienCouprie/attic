...
export const fiches: FicheAudio[] = ([
  {
    ...
    notice: "D'après la famille `GRAIN` du Composers Desktop Project, développée par Trevor Wishart. Ce qui la distingue de tout le reste du catalogue granulaire : les frontières viennent du son et non d'une cadence réglée. Le gel granulaire, le brassage, la découpe aléatoire et les particules découpent sur une grille qui ne doit rien à ce qu'on lui donne ; ici, chaque frappe d'un rythme, chaque syllabe d'une voix, chaque note d'un arpège devient un grain que l'on peut retirer, déplacer ou répéter sans jamais couper au milieu d'un son.\n\nLa détection tient à deux règles, et la seconde est celle qui fait le travail. La première sépare ce qui sonne de ce qui ne sonne pas, au seuil en décibels, avec l'enveloppe du rognage des silences — deux nœuds qui découpent le même son ne doivent pas le découper autrement. Mais cette règle échoue exactement là où le procédé sert : sur un roulement, une phrase tenue ou une nappe, l'enveloppe ne redescend jamais au silence et l'on obtient un grain unique de trente secondes. La seconde règle cherche donc les attaques à l'intérieur de ce qui sonne : une remontée franche de l'enveloppe après une retombée ouvre un grain, même sans silence devant.\n\nLa sensibilité se règle, et elle doit l'être. Elle dit de combien de décibels l'enveloppe doit remonter pour qu'on y voie une attaque. Mesuré sur un roulement dont les frappes tombent toutes les 240 millisecondes sous une décroissance de 250 : entre deux d'entre elles, l'enveloppe ne retombe que de 8,4 décibels. À neuf décibels de sensibilité, une frappe se cache dans la queue de la précédente ; à six, les huit ressortent. Le réglage se tend vers le son qu'on lui donne, il ne devine pas.\n\nL'écart minimal n'est pas un confort. Une attaque n'est pas un instant mais une montée de quelques millisecondes, où l'enveloppe tremble : sans lui, une seule frappe donne trois ou quatre grains et toutes les manipulations deviennent du hachis.\n\nCommencez par « Compter seulement ». L'opération ne touche pas au son et le rapport dit ce qui a été trouvé — combien de grains, leur durée moyenne, leur écart moyen, et la liste de leurs départs en secondes. C'est là qu'on règle le seuil et la sensibilité ; tout ce qui suit en dépend.\n\nL'espacement décide de ce que devient le rythme. « D'origine » repose chaque grain à l'instant où il a été trouvé : retirer une frappe sur deux laisse un trou à sa place, la durée ne bouge pas, et le rythme reste reconnaissable — c'est ce qu'on veut pour éclaircir une boucle. « Serré » recolle les grains bout à bout : le son raccourcit d'autant et le rythme change. Le premier conserve la durée, le second la densité.\n\nInverser l'ordre retourne la suite des grains sans retourner les grains eux-mêmes : les frappes se succèdent à l'envers, chacune restant à l'endroit. C'est tout autre chose qu'une lecture inversée, qui rendrait chaque attaque en fin de son.",
    noticeEn: "After the `GRAIN` family of the Composers Desktop Project, developed by Trevor Wishart. What sets it apart from the rest of the granular catalog: the boundaries come from the sound and not from a set rate. Granular freeze, brassage, random slicing and particles all cut on a grid that owes nothing to what it is given; here each hit of a rhythm, each syllable of a voice, each note of an arpeggio becomes a grain that can be dropped, moved or repeated without ever cutting in the middle of a sound.\n\nDetection rests on two rules, and the second does the work. The first separates what sounds from what does not, by a threshold in decibels, using the envelope of the silence trimmer — two nodes cutting the same sound should not cut it differently. But that rule fails exactly where the process is useful: on a roll, a held phrase or a pad, the envelope never falls back to silence and you get one single thirty-second grain. The second rule therefore looks for attacks inside what sounds: a clear rise of the envelope after a fall opens a grain, with no silence in front of it.\n\nSensitivity is a setting, and it has to be. It says by how many decibels the envelope must rise for an attack to be seen. Measured on a roll whose hits fall every 240 milliseconds under a 250 millisecond decay: between two of them the envelope only falls by 8.4 decibels. At nine decibels of sensitivity one hit hides in the previous one's tail; at six, all eight come out. The setting reaches towards the sound it is given, it does not guess.\n\nThe minimum gap is no convenience. An attack is not an instant but a rise of a few milliseconds, where the envelope wavers: without it a single hit gives three or four grains and every manipulation turns to mincemeat.\n\nStart with « Count only ». The operation does not touch the sound, and the report says what was found — how many grains, their mean duration, their mean gap, and the list of their onsets in seconds. That is where threshold and sensitivity get set; everything that follows depends on them.\n\nSpacing decides what becomes of the rhythm. « As found » puts each grain back at the instant it was found: dropping one hit in two leaves a hole in its place, duration does not move, and the rhythm stays recognisable — which is what you want to thin out a loop. « Butted » glues the grains end to end: the sound shortens accordingly and the rhythm changes. The first keeps duration, the second keeps density.\n\nReversing the order turns the sequence of grains around without turning the grains themselves: the hits follow one another backwards, each of them still forwards. That is a wholly different thing from reversed playback, which would put every attack at the end of its sound.",
