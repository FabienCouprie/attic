...
export const fiches: FicheAudio[] = ([
  {
    ...
    notice: "Synthétise des grains sonores par un générateur unique, réglé en sept espèces que le paramètre « Espèce » sélectionne. D'après Øyvind Brandtsegg, Sigurd Saue et Thom Johansen, « Particle synthesis : a unified model for granular synthesis », Linux Audio Conference, 2011, et l'opcode `partikkel` de Csound : les variétés de synthèse granulaire n'y sont pas des techniques distinctes, mais un même générateur réglé autrement.\n\nLes grains sont le cas ordinaire : une forme d'onde brève, répétée à une cadence. La cadence donne la hauteur entendue dès qu'elle dépasse une vingtaine par seconde ; en dessous, on compte les grains.\n\nLes pulsars découplent deux durées. Chez eux, la durée du grain se compte en cycles de la forme d'onde au lieu de suivre la cadence : la cadence fixe alors la fondamentale, et la fréquence fixe le formant, indépendamment l'un de l'autre.\n\nLes glissons donnent à chaque grain sa propre trajectoire de fréquence : le grain n'est plus un point mais un vecteur. Le réglage de transposition dit de combien il monte ou descend pendant sa brève existence.\n\nLes trainlets remplacent la forme d'onde par un train d'impulsions à bande limitée, dont on règle le nombre de partiels. Ils sonnent comme des clics qui ont une hauteur, à douze partiels ils portent près de 27 % de leur énergie au-dessus de quatre kilohertz, contre deux dixièmes de pour-cent pour un grain sinusoïdal, et leur facteur de crête passe de 6 à 16 décibels.\n\nLes grainlets lient deux réglages l'un à l'autre, ce qui est leur seule définition. La fréquence parcourt l'intervalle donné par la transposition au fil de la note, et la durée du grain la suit : chaque grain porte alors le même nombre de cycles, les aigus courts et les graves longs, comme le fait une ondelette. Sans cette liaison, un balayage de deux octaves laisserait les grains graves à quatre cycles et les aigus à seize, et l'on entendrait le grain changer de nature en chemin. Elle ne fabrique pas un timbre nouveau : elle empêche un timbre de dériver.\n\nLa granulation prend le son branché sur l'entrée et le lit par grains. C'est là que la position et la vitesse servent : à vitesse nulle, la tête de lecture ne bouge plus et l'on obtient un gel granulaire ; à un demi, le son dure deux fois plus longtemps sans changer de hauteur ; en négatif, il se lit à l'envers. Sans son branché, l'espèce retombe sur les grains, ce que le composant signale.\n\nLa granulation synchrone cale les grains sur la période du son au lieu d'une grille régulière. Une grille impose sa cadence au son : sur une scie à 220 hertz, granuler à soixante grains par seconde ne laisse plus aucune hauteur tenue, et à cent cinquante la hauteur tombe à 73 hertz, celle de la grille, non celle du son. Calée sur la période, la même granulation rend 220,14 hertz, soit la hauteur d'origine à trois centièmes de hertz près. La période est lue par un suiveur pYIN. Dans cette espèce, la densité et la dispersion ne commandent rien, puisque c'est le son qui donne la cadence ; la durée du grain, elle, se compte en périodes.\n\nLa largeur spatiale éparpille les grains entre les deux canaux : chacun reçoit sa propre place au lieu que le son entier se déplace d'un bloc. L'oreille y entend un nuage, et non une direction. À cent vingt grains par seconde, la corrélation des deux canaux passe de 1,00 à 0,31 entre largeur nulle et pleine ouverture ; l'équilibre reste nul, et la sonie ne bouge pas, à −16,91 LUFS. Les positions vont par paires miroir, et l'amplitude est corrigée de ce que l'éparpillement ajouterait d'énergie, de sorte que la largeur ne change pas le niveau.\n\nLa dispersion désordonne les instants. À zéro, les grains tombent sur une grille régulière et l'oreille entend une hauteur ; en montant, la grille se brouille et la hauteur cède la place à une texture. C'est le passage du synchrone à l'asynchrone, qui se fait ici par un seul curseur.",
    noticeEn: "Synthesises sound grains with a single generator, set in seven species selected by the « Species » parameter. After Oyvind Brandtsegg, Sigurd Saue and Thom Johansen, « Particle synthesis: a unified model for granular synthesis », Linux Audio Conference, 2011, and Csound's `partikkel` opcode: the varieties of granular synthesis are not distinct techniques there, but one generator set differently.\n\nGrains are the ordinary case: a brief waveform, repeated at a rate. The rate gives the pitch heard as soon as it passes some twenty per second; below that, grains can be counted.\n\nPulsars decouple two durations. For them the grain duration counts in cycles of the waveform instead of following the rate: the rate then sets the fundamental, and the frequency sets the formant, independently of each other.\n\nGlissons give each grain its own frequency trajectory: the grain is no longer a point but a vector. The transposition setting says how far it rises or falls during its brief existence.\n\nTrainlets replace the waveform with a band-limited impulse train, whose number of partials is set. They sound like clicks that have a pitch: at twelve partials they carry close to 27 % of their energy above four kilohertz, against two tenths of a percent for a sine grain, and their crest factor rises from 6 to 16 decibels.\n\nGrainlets tie two settings to one another, which is their whole definition. Frequency travels the interval given by the transposition across the note, and grain duration follows it: each grain then carries the same number of cycles, high ones short and low ones long, as a wavelet does. Without that link, a two-octave sweep would leave the low grains at four cycles and the high ones at sixteen, and the grain would be heard changing nature on the way. It does not make a new timbre; it stops a timbre from drifting.\n\nGranulation takes the sound connected to the input and reads it in grains. That is where position and speed serve: at zero speed the read head stops moving and you get a granular freeze; at one half the sound lasts twice as long without changing pitch; negative, it reads backwards. With no sound connected the species falls back to grains, and the node says so.\n\nPitch-synchronous granulation locks the grains to the sound's period instead of a regular grid. A grid imposes its own rate on the sound: on a 220 hertz sawtooth, granulating at sixty grains per second leaves no sustained pitch at all, and at a hundred and fifty the measured pitch drops to 73 hertz, the grid's, not the sound's. Locked to the period, the same granulation returns 220.14 hertz, the original pitch to within three hundredths of a hertz. The period is read by a pYIN follower. In this species, density and dispersion command nothing, since the sound gives the rate; grain duration, for its part, counts in periods.\n\nSpatial width scatters the grains between the two channels: each one gets its own place instead of the whole sound moving as a block. The ear hears a cloud from it, not a direction. At a hundred and twenty grains per second, the correlation of the two channels goes from 1.00 to 0.31 between zero width and full opening; balance stays at nil, and loudness does not move, at -16.91 LUFS. Positions come in mirrored pairs, and amplitude is corrected for what the scattering would add in energy, so that width does not change the level.\n\nDispersion disorders the instants. At zero, grains land on a regular grid and the ear hears a pitch; going up, the grid blurs and pitch gives way to texture. That is the passage from synchronous to asynchronous, done here with one slider.",
    ...
    parametres: [
      ...
      { nom: "Densité", nomEn: "Density", type: "curseur", plage: [1, 500], pas: 1, defaut: 60, unite: "grains/s",
        doc: "Grains par seconde : sans effet sur la granulation synchrone, dont la cadence vient du son. Sous une vingtaine, on les compte et le composant sonne comme un rythme ; au-dessus, ils fusionnent et la cadence devient une hauteur ; c'est le continuum entre rythme et hauteur, franchi par un seul curseur.",
