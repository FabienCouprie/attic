...
export const fiches: FicheAudio[] = ([
  ...
  },
  {
    ...
    notice: "Un fichier chargé depuis le disque arrive avec un nombre de canaux, et rien d'autre. Or quatre canaux, c'est une quadriphonie ou une ambisonie d'ordre un : même nombre, deux mondes, et les traiter l'un comme l'autre rend du bruit. Attic ne devine donc jamais à partir du seul nombre, et c'est ce nœud qui le dit.\n\nLe nombre de canaux de l'entrée doit correspondre à la disposition déclarée ; sinon le nœud refuse, plutôt que de laisser un 7.1 passer pour un anneau de huit. Le son n'est pas modifié : seule l'étiquette change, et elle suit ensuite le son jusqu'à l'export.",
    noticeEn: "A file loaded from disk arrives with a number of channels, and nothing else. Yet four channels is a quad or a first-order ambisonic field: the same number, two worlds, and treating one as the other gives noise. Attic therefore never guesses from the number alone, and this node is what states it.\n\nThe input's channel count must match the declared layout; otherwise the node refuses, rather than letting a 7.1 pass for a ring of eight. The sound is not changed: only the label is, and it then follows the sound to the export.",
  ...
  },
  {
    ...
    notice: "On n'écoute pas un 7.1.4 au casque, et c'est pourtant au casque qu'on compose. Sans ce nœud, un espace écrit pour douze haut-parleurs reste inaudible sur le poste de travail : le lecteur du navigateur replie tout en stéréo par ses propres règles, qui ne savent rien de l'ambisonie.\n\nChaque canal devient un haut-parleur virtuel placé à sa direction, filtré par les fonctions de transfert de la tête du moteur audio. Un champ ambisonique est d'abord décodé vers vingt-six directions réparties sur la sphère. Le procédé est le même pour les quatre familles, de sorte qu'on compare au casque des choses comparables.\n\nC'est une écoute de travail, pas un export : le caisson de graves est ajouté aux deux oreilles six décibels plus bas, et si la somme dépasse le plein calibre tout le rendu est ramené juste en dessous — un écrêtage masquerait précisément ce qu'on vient écouter. Les fonctions de transfert sont celles du moteur, génériques : elles ne sont pas celles de votre tête, et l'impression d'avant et d'arrière en souffre toujours un peu.",
