...
export const fiches: FicheAudio[] = ([
  ...
  },
  {
    ...
    notice: "Une réverbération dont la queue monte d'une octave à chaque tour, et qui s'éloigne en montant. L'effet est associé aux nappes de Brian Eno et Daniel Lanois, et son principe est une boucle : la queue est transposée puis réinjectée dans la réverbération, indéfiniment, chaque tour plus haut et plus faible.\n\nUn graphe acyclique ne peut pas l'exprimer ; c'est une rétroaction, pas une chaîne, et c'est pourquoi cela demande un composant plutôt qu'un câblage. La boucle est donc déroulée en générations : la première est la réverbération du son, la deuxième celle de la première transposée, et ainsi de suite. Quatre ou cinq suffisent ; au-delà, tout est sous le plancher d'audition, et le composant affiche le niveau de chacune pour qu'on le voie.\n\nLa première génération n'est pas transposée, et c'est ce qui rend l'effet reconnaissable : on entend d'abord la salle, puis l'octave qui monte dedans. Transposer dès le premier tour donnerait un son aigu immédiat.\n\nLa réponse de réverbération est normalisée en énergie. Sans cela, chaque convolution ajouterait de l'énergie : la quatrième génération sortirait soixante-quatorze décibels au-dessus de la première, et le réglage de rebouclage ne commanderait rien. Le rebouclage décide seul de l'extinction.",
    noticeEn: "A reverb whose tail rises an octave at each pass, receding as it climbs. The effect is associated with Brian Eno's and Daniel Lanois's pads, and its principle is a loop: the tail is transposed then fed back into the reverb, indefinitely, each pass higher and quieter.\n\nAn acyclic graph cannot express that, feedback not being a chain, which is why it takes a node rather than a patch. The loop is therefore unrolled into generations: the first is the reverb of the sound, the second the reverb of the first transposed, and so on. Four or five are enough; beyond that everything is below the hearing floor, and the node shows each one's level so it can be seen.\n\nThe first generation is not transposed, and that is what makes the effect recognisable: one hears the room first, then the octave rising inside it. Transposing from the first pass would give an immediate high sound.\n\nThe reverb response is normalised in energy. Without that, every convolution would add energy: the fourth generation would come out seventy-four decibels above the first, and the feedback setting would command nothing. Feedback alone decides the decay.",
    entrees: [{ nom: "Audio", type: "audio" }],
    sorties: [{ nom: "Audio", type: "audio" }],
    parametres: [
      ...
      { nom: "Générations", nomEn: "Generations", type: "curseur", plage: [1, 8], pas: 1, defaut: 4,
        doc: "Nombre de tours déroulés. Au-delà de quatre ou cinq, tout est sous le plancher d'audition et le calcul coûte pour rien ; le composant affiche le niveau de chaque génération pour qu'on le voie.",
