...
export const fiches: FicheAudio[] = ([
  {
    ...
    notice: "Corrige la justesse d'un son en ramenant chaque note sur le degré le plus proche d'une gamme. Le composant rassemble un suiveur de hauteur pYIN, qui rend une fréquence par trame avec sa confiance, des gammes, des tempéraments, et de quoi transposer.\n\nDeux réglages font tout, et on les confond volontiers. La force dit quelle part de l'écart est corrigée : à 100 %, la note tombe pile sur le degré ; à 50 %, la moitié du vibrato et des attaques est conservée. La transition dit en combien de temps la correction s'installe : à zéro, la hauteur saute d'un degré à l'autre sans passer par les intermédiaires ; c'est l'effet rendu célèbre par un disque de 1998 ; à cinquante millisecondes, l'oreille n'entend plus qu'une justesse retrouvée.\n\nLa méthode est le recollement synchrone des périodes, et ce choix a une conséquence qui s'entend : les formants ne suivent pas la note. Le signal est découpé en grains de deux périodes qu'on recolle à un espacement différent ; le contenu de chaque grain ne bouge pas, si bien qu'une voix corrigée ne prend pas l'accent de l'écureuil, ce qu'un rééchantillonnage lui aurait fait.\n\nSa limite, puisqu'elle en a une : elle suppose un son périodique. Sur une voix ou un instrument tenu, c'est le cas ; sur un accord, un bruit ou une percussion, il n'y a pas de période à recoller, et le seuil de confiance est là pour que ces passages ne soient pas corrigés du tout. Le rapport est borné à quatre demi-tons : au-delà, deux copies d'un même grain s'annulent, à l'octave la raie dominante tombe à un centième du niveau d'entrée. Une transposition franche relève d'un autre traitement.\n\nL'écart maximal borne la correction : un suiveur de hauteur se trompe d'octave sur les sons riches, et corriger une erreur d'octave déplacerait la note d'une octave entière. Au-delà de la borne, aucune correction n'est appliquée.\n\nLa seconde sortie rend la correction appliquée sous forme de courbe : un demi vaut aucune correction, le haut tire vers l'aigu, le bas vers le grave. La courbe montre ce que le composant a corrigé, et où il a renoncé.",
    noticeEn: "Corrects the tuning of a sound by bringing each note onto the nearest degree of a scale. The node brings together a pYIN pitch follower, returning a frequency per frame with its confidence, scales, temperaments, and ways to transpose.\n\nTwo settings do everything, and they are readily confused. Strength says how much of the deviation is corrected: at 100 % the note lands exactly on the degree; at 50 % half the vibrato and the attacks are kept. Transition says how long the correction takes to settle: at zero the pitch jumps from one degree to the next without passing through, the effect made famous by a 1998 record; at fifty milliseconds the ear hears nothing but tuning restored.\n\nThe method is pitch-synchronous overlap-add, and that choice has an audible consequence: formants do not follow the note. The signal is cut into two-period grains that are glued back at a different spacing; each grain's content does not move, so a corrected voice does not take on a chipmunk accent, which resampling would have done to it.\n\nIts limit, since it has one: it assumes a periodic sound. On a voice or a sustained instrument that holds; on a chord, a noise or a drum there is no period to glue, and the confidence threshold is there so those passages are not corrected at all. The ratio is bounded to four semitones: beyond that, two copies of one grain cancel: at the octave the dominant line falls to a hundredth of the input level. Transposing in earnest is another kind of processing.\n\nThe maximum deviation bounds the correction: a pitch follower makes octave errors on rich sounds, and correcting an octave error would move the note by a whole octave. Beyond the bound nothing is corrected.\n\nThe second output returns the applied correction as a curve: a half means no correction, the top pulls towards the treble, the bottom towards the bass. The curve shows what the node corrected, and where it gave up.",
