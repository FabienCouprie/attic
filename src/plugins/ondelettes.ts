...
export const fiches: FicheAudio[] = ([
  {
    ...
    notice: "Analyse un son par ondelettes, seuille ses coefficients et le reconstruit. D'après Ingrid Daubechies, « Orthonormal bases of compactly supported wavelets », Communications on Pure and Applied Mathematics 41(7), 1988 ; le seuillage vient de David Donoho et Iain Johnstone, « Ideal spatial adaptation by wavelet shrinkage », Biometrika 81(3), 1994, et la moyenne sur des décalages de Ronald Coifman et David Donoho, « Translation-invariant de-noising », 1995.\n\nCe qu'une ondelette a que la transformée de Fourier n'a pas. Une transformée à court terme impose une fenêtre unique à tout le son : longue, elle sépare finement les hauteurs et étale les attaques ; courte, elle place les attaques et confond les graves. Il faut choisir une fois pour toutes, et le choix est toujours mauvais quelque part. Une ondelette ne choisit pas : elle regarde l'octave la plus aiguë à travers une fenêtre courte, la suivante à travers une fenêtre deux fois plus longue, et ainsi de suite jusqu'au grave. Les fenêtres sont donc courtes dans l'aigu et longues dans le grave, exactement et de façon réversible.\n\nLa reconstruction est parfaite. Sans toucher aux coefficients, le composant rend le son d'entrée au dix-millionième près : c'est ce que rend le mode « Reconstruire », pour chacune des quatre ondelettes, y compris sur du bruit.\n\nCe que le seuillage apporte. Le bruit se répand sur tous les coefficients ; un son structuré se concentre dans quelques-uns. Effacer les petits coefficients efface donc surtout du bruit. Sur une note tenue, couper l'aigu suffirait à gagner des décibels. Le cas qui tranche est celui des clics à large bande, qu'aucune coupure de bande ne peut nettoyer : une annulation complète des mêmes bandes y gagne 0,01 dB, le seuillage 11 dB. La différence est ce que la parcimonie apporte, et rien d'autre.\n\nLes décalages. Une transformée décimée n'est pas invariante par translation : le même son avancé d'un échantillon ne donne pas les mêmes coefficients, et il reste autour des attaques un fourmillement caractéristique. Traiter le son à plusieurs décalages et faire la moyenne l'efface, puisque les artefacts dépendent du décalage et le son non. Quatre décalages suffisent en général ; au-delà, le coût monte plus vite que le gain.\n\nAvec peu d'étages, le grave entier échappe au traitement : le composant se comporte alors en partie comme un filtre, et un son à bande étroite paraîtra nettoyé sans que les ondelettes y soient pour grand-chose. Le réglage doit aller chercher assez d'étages pour que la bande du son soit elle-même seuillée.\n\nLa seconde sortie rend exactement ce qui a été retiré. Si l'on y entend le son et non le souffle, le seuil est trop fort.",
    noticeEn: "Analyses a sound by wavelets, thresholds its coefficients and reconstructs it. After Ingrid Daubechies, « Orthonormal bases of compactly supported wavelets », Communications on Pure and Applied Mathematics 41(7), 1988; thresholding comes from David Donoho and Iain Johnstone, « Ideal spatial adaptation by wavelet shrinkage », Biometrika 81(3), 1994, and averaging over shifts from Ronald Coifman and David Donoho, « Translation-invariant de-noising », 1995.\n\nWhat a wavelet has that the Fourier transform has not. A short-term transform imposes one single window on the whole sound: long, it separates pitches finely and smears attacks; short, it places attacks and blurs the bass. One must choose once and for all, and the choice is always wrong somewhere. A wavelet does not choose: it looks at the highest octave through a short window, the next one through a window twice as long, and so on down to the bass. The windows are therefore short in the treble and long in the bass, exactly and reversibly.\n\nReconstruction is perfect. Without touching the coefficients, the node returns the input sound to within a ten-millionth: that is what the « Reconstruct » mode returns, for each of the four wavelets, noise included.\n\nWhat thresholding brings. Noise spreads over every coefficient; a structured sound concentrates into a few. Erasing the small coefficients therefore erases mostly noise. On a held note, cutting the treble alone would win decibels. The case that settles it is broadband clicks, which no band cut can clean: zeroing the very same bands gains 0.01 dB there, and thresholding 11 dB. The difference is what sparsity brings, and nothing else.\n\nThe shifts. A decimated transform is not translation invariant: the same sound moved forward by one sample does not give the same coefficients, and a characteristic shimmer is left around attacks. Processing the sound at several shifts and averaging removes it, since the artefacts depend on the shift and the sound does not. Four shifts are usually enough; beyond that, cost rises faster than gain.\n\nWith few levels, the whole bass escapes processing: the node then behaves partly as a filter, and a narrowband sound will seem cleaned without wavelets having much to do with it. The setting must reach for enough levels that the sound's own band is thresholded too.\n\nThe second output returns exactly what was removed. If the sound is heard there rather than the hiss, the threshold is too strong.",
