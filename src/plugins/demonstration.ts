...
export const fiches: FicheAudio[] = ([
  {
    ...
    resume: "Fabrique la vidéo du graphe où il est posé : chaque composant à son tour, son nom, ses réglages, et son résultat joué ou affiché.",
    resumeEn: "Makes a video of the graph it sits in: each node in turn, its name, its settings, and its result played or shown.",
    notice: "Ce composant fabrique une vidéo du graphe où il est posé. Il ne se branche à rien : il passe après tous les autres composants, reprend leurs résultats et consacre à chacun un segment de même durée.\n\nLes composants sont montrés en suivant les chaînes du graphe : un composant vient juste après celui qui l'alimente, avant de passer à une autre branche. Aucun composant n'est montré avant ce qu'il reçoit.\n\nChaque segment montre le nom du composant, son résumé, ses réglages, et son résultat. Un son est joué, et sa forme d'onde défile sous une tête de lecture ; une courbe est tracée et parcourue ; une image est affichée ; un texte est affiché, et défile s'il dépasse le cadre. Un composant qui rend plusieurs choses est montré par son son d'abord, puis son image, sa courbe, son texte. Un composant qui ne rend rien de montrable n'a pas de segment. En bas de l'image, la chaîne des étapes situe le composant courant : un trait relie deux étapes quand la première alimente la seconde, un point les sépare sinon.\n\nLe son d'un composant commence 0,4 s après le début de son segment et s'arrête 0,3 s avant la fin ; un son plus long est coupé avec un fondu, et la légende indique l'extrait joué. Les sons sont posés tels quels, sans normalisation : on entend les niveaux que le graphe produit.\n\nLa vidéo est rendue hors temps réel, image par image, et encodée en WebM (vidéo VP9, son Opus à 48 kHz) : deux rendus du même graphe donnent la même vidéo. Lancer ce composant seul lance tout le graphe. Les copies d'un composant dans une boucle n'ont qu'un segment, qui montre le résultat du dernier tour.",
    ...
    parametres: [
      { nom: "Durée par nœud", nomEn: "Length per node", type: "curseur", plage: [2, 60], pas: 0.5, defaut: 6, unite: "s",
        doc: "Durée du segment de chaque composant. Un son plus long que le segment, moins 0,7 s, est coupé.",
      ...
      { nom: "Titre", nomEn: "Title", type: "texte", defaut: "", defautEn: "", placeholder: "Une voix, trois traitements", placeholderEn: "One voice, three treatments",
        doc: "Titre affiché sur un carton d'ouverture de 3 s. Vide, la vidéo commence directement par le premier composant.",
      ...
      { nom: "Réglages", nomEn: "Settings", type: "choix", options: ["Affichés", "Masqués"], optionsEn: ["Shown", "Hidden"],
        ...
        doc: "Affiche à droite de chaque segment les valeurs réglées du composant, huit au plus. Masqués, le résultat prend toute la largeur.",
  ...
  },
  {
    ...
    univers: "Autres", famille: "Vidéo",
    resume: "Filme la fenêtre de l'application pendant qu'elle construit le graphe où il est posé, le lance et fait entendre chaque composant.",
    resumeEn: "Films the application window while it builds the graph it sits in, runs it and plays each node.",
    notice: "Ce composant filme la fenêtre de l'application pendant qu'elle rejoue le graphe où il est posé. Il ne se branche à rien ; le film se lance par son bouton « Filmer l'application », et non par l'exécution du graphe.\n\nD'abord la construction : le canevas se vide, puis chaque composant apparaît à sa place, avec en légende son nom et l'endroit de la palette où il se trouve ; ses câbles se tirent depuis les composants déjà posés, et ses réglages s'ouvrent dans l'inspecteur. Les composants sont posés en suivant les chaînes du graphe : un composant vient juste après celui qui l'alimente. Puis la visite : le graphe est lancé, la vue s'approche de chaque composant à son tour, l'inspecteur montre ses réglages, et son résultat se fait entendre pendant la durée par composant. Un curseur dessiné montre chaque geste. Le film finit sur le graphe entier.\n\nLe titre, s'il est rempli, ouvre le film. Échap interrompt. À la fin, le graphe, la vue et la sélection sont rendus tels qu'ils étaient, avec les résultats du run. Dans l'application, la fenêtre se filme elle-même ; dans un navigateur, celui-ci demande l'autorisation de partager l'onglet. Le son enregistré est celui des résultats écoutés. Le film est un WebM, à la taille de la fenêtre.",
    noticeEn: "This node films the application window while it replays the graph it sits in. It connects to nothing; the film starts from its « Film the application » button, not from running the graph.\n\nFirst the construction: the canvas empties, then each node appears in its place, captioned with its name and where it sits in the palette; its cables are drawn from the nodes already placed, and its settings open in the inspector. Nodes are placed following the chains of the graph: a node comes right after the one that feeds it. Then the tour: the graph runs, the view moves close to each node in turn, the inspector shows its settings, and its result is heard for the length per node. A drawn cursor shows every gesture. The film ends on the whole graph.\n\nThe title, if filled in, opens the film. Esc stops it. At the end, the graph, the view and the selection are put back as they were, with the results of the run. In the application, the window films itself; in a browser, the browser asks permission to share the tab. The recorded sound is that of the results played. The film is a WebM, at the size of the window.",
    entrees: [],
    sorties: [],
    sortieNullePermise: true,
    parametres: [
      { nom: "Durée par nœud", nomEn: "Length per node", type: "curseur", plage: [1, 60], pas: 0.5, defaut: 5, unite: "s",
        doc: "Temps d'écoute de chaque composant pendant la visite. Un son plus court s'arrête avant ; un composant qui ne rend ni son ni image est passé.",
