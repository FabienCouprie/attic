...
export const fiches: FicheAudio[] = ([
  {
    ...
    parametres: [
      { nom: "Modèle", nomEn: "Model", type: "texte", defaut: "qwen3:4b",
        ...
        docEn: "Installed Ollama model name (see « ollama list »). E.g. llama3.2, qwen3:4b, mistral, phi3.", defautEn: "qwen3:4b" },
      { nom: "Prompt", nomEn: "Prompt", type: "texte", defaut: "Write the lyrics of a short song about the sea and freedom.",
        ...
        docEn: "Instruction sent to the model. Ignored if a text input is connected.", defautEn: "Write the lyrics of a short song about the sea and freedom." },
    ...
    async executer(ctx: any) {
      ...
      if (!prompt.trim()) return { valeurs: [null], message: traduire("msg.aucun_prompt_param_tre_ou_entr_e_texte") };
      const model = ctx.paramTexte("Modèle", "llama3.2").trim() || "llama3.2";
      ctx.onProgress(traduire("progress.ollama_var_0", model));
      ...
      if (res.erreur) return { valeurs: [null], erreur: true, message: traduire("msg.ollama_var_0", res.erreur) };
