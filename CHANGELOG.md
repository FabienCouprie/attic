...
### Added
- **Nouveau sous-menu Autres → Algèbre musicale : nœud « Classification de pistes ».** Regroupe une collection de pistes audio par similarité : chaque piste est réduite à un vecteur de 40 features (tempo, centroïde spectral, chroma, MFCC moyenne/variance), standardisées puis projetées par PCA (implémentation maison — diagonalisation par balayages de Jacobi, aucune dépendance externe). Un KMeans (k fixe ou recherche automatique par indice de Calinski-Harabasz, premier maximum local plutôt que le score maximal global) regroupe les pistes dans l'espace réduit ; un mélange de gaussiennes (GMM, initialisé sur ce partitionnement) estime ensuite la probabilité d'appartenance de chaque piste à chaque groupe, avec régularisation des covariances quasi-singulières (peu de pistes par groupe). Quatre sorties : un nuage de points SVG interactif sur « Graphique » (type image, chaînable vers Export SVG ou Rendu image comme sur Camelot — 2 premiers axes PCA, couleur+forme par groupe, nom de piste au survol), un tableau SVG lisible sur « Moyennes par groupe » (variables de départ en ligne, groupes en colonne — peu de groupes en pratique contre 40 variables, colonnes étroites à texte pivoté écartées d'emblée ; moyenne de chaque variable par groupe, colorée par écart à la moyenne inter-groupes de sa ligne — repère en un coup d'œil quelles variables et quels groupes portent la différence, plutôt que de creuser le JSON à la main ; chaînable vers Rendu image, redimensionnable jusqu'à 1200×800, ou vers Export SVG pour l'ouvrir dans un vrai navigateur et zoomer sans perte), et deux flux texte JSON structurés (rapport de classification, coordonnées) réutilisables par un autre nœud ou logiciel. Nécessite au moins 3 pistes décodables.
